...
namespace CanvasManager.CanvasShapes
{
    ...
    public partial class CanvasLayoutArea : GraphicsLayoutArea, IAreaShape
    {
        ...
        public string fileLocation = "CanvasManager.CanvasShape.CanvasLayoutArea";
        ...
        public GraphicShape DrawCompositeShape(GraphicsWindow window, GraphicsPage page, bool drawPolygonOnly = false)
        {
            ...
            if (IsSeamed())
            {
                if (SeamIndexTag != null)
                {
                    ...
                    this.AreaFinishManager.SeamDesignStateLayer.AddShape(SeamIndexTag, 1);
                }
            }
            ...
            GraphicShape shape = VisioGeometryEngine.Subtract(this, window, page, externalArea.Shape, shapes);
        ...
        public void SetCopyMarker(double x, double y)
        {
            ...
            //copyMarkerShape.AddToLayerSet(CopyMarker);
        ...
        public void RegenerateSeamsAndCuts()
        {
        ...
        public void GenerateSeamsAndRollouts(CanvasDirectedLine line)
        {
        ...
        public void GenerateSeamsAndRollouts(DirectedLine line)
        {
            if (this.LayoutAreaType == LayoutAreaType.Normal)
            {
        ...
        public void GenerateSeamsAndRolloutsNormal(DirectedLine line)
        { 
            ...
            foreach (GraphicsRollout graphicsRollout in GraphicsRolloutList)
            {
                foreach (GraphicsCut graphicsCut in graphicsRollout.GraphicsCutList)
                {
                    ...
                    this.Page.AddToPageShapeDict(cutShape);
                   ...
                   // Page.CutsLayer.Add(cutShape.cutin, 1);
                    ...
                    foreach (GraphicsOverage overage in graphicsCut.GraphicsOverageList)
                    {
                        ...
                        {
                            Page.AddToPageShapeDict(overage.GraphicsOverageIndex);

                            areaFinishLayers.OversIndexLayer.AddShape(overage.GraphicsOverageIndex, 1);
            ...
            //canvasManager.UpdateAreaAndSeamsStats();
             SystemGlobals.UpdateAreaSeamsUndrsOversDataDisplay();
        }

        public void GenerateSeamsAndRolloutsOversGenerator(DirectedLine line)
        {
            DeleteNonManualSeams();
            ...
            foreach (GraphicsRollout graphicsRollout in GraphicsRolloutList)
            {
                ...
                {
                    GraphicsOverage graphicsOverage = new GraphicsOverage((GraphicsCut) overage.ParentCut, Window, Page, FinishLibElements, overage);
        ...
        public void UpdateOverIndicesAndVisibility(ref uint overIndx)
        {
            ...
            List<Tuple<GraphicsOverageIndex, GraphicsOverage>> graphicsOverageIndexList = new List<Tuple<GraphicsOverageIndex, GraphicsOverage>>();
            ...
            foreach (GraphicsOverage graphicsOverage in this.GraphicsOverageList)
            {
                ...
                {
                    graphicsOverageIndexList.Add(new Tuple<GraphicsOverageIndex, GraphicsOverage>(graphicsOverageIndex, graphicsOverage));
                }
            }
            ...
            foreach (Tuple<GraphicsOverageIndex, GraphicsOverage> listElem in graphicsOverageIndexList)
            {
                overIndx = Overage.OverageIndexGenerator();
            }
        ...
        public void UpdateCutIndicesAndVisibility(ref uint cutIndex)
        {
            ...
            foreach (GraphicsCut graphicsCut in this.GraphicsCutList)
            {
                ...
                //{
                //    graphicsCutIndexList.Add(graphicsCutIndex);
                //}
            }

            lclGraphicsCutList.Sort((i1, i2) => Coordinate.Compare(i1.GraphicsCutIndex.Location, i2.GraphicsCutIndex.Location));
            ...
            foreach (GraphicsCut graphicsCut in lclGraphicsCutList)
            {
                ...
                graphicsCut.GraphicsCutIndex.SetVisibility(true);
            }
        }


        public void UpdateCutOverAndUnderIndices(ref uint cutIndex, ref uint overIndx, ref uint undrIndx)
        {
            ...
            List<GraphicsOverage> graphicsOverageIndexList = new List<GraphicsOverage>();
            ...
            foreach (GraphicsOverage graphicsOverage in this.GraphicsOverageList)
            {
                ...
                if (Utilities.IsNotNull(graphicsOverageIndex))
                {
                    graphicsOverageIndexList.Add(graphicsOverage);
                }
            }

            graphicsOverageIndexList.Sort((i1, i2) => Coordinate.Compare(i1.GraphicsOverageIndex.Location, i2.GraphicsOverageIndex.Location));

            // Now we number them
            foreach (GraphicsOverage graphicsOverage in graphicsOverageIndexList)
            {
                uint overageIndex = graphicsOverage.OverageIndex;

                Overage.RemoveIndex(overageIndex);
            }

            foreach (GraphicsOverage grapicsOverage in graphicsOverageIndexList)
            {
                uint overageIndex = Overage.OverageIndexGenerator();
                grapicsOverage.OverageIndex = overageIndex;

                GraphicsOverageIndex graphicsOverageIndex = grapicsOverage.GraphicsOverageIndex;
        ...
        public void GenerateSeamsAndRolloutsRemnant(DirectedLine line)
        {
            DeleteNonManualSeams();
            ...
            double remnantSeamWidthInFeet = CurrentPage.RemnantSeamWidthInFeet();

            rollWidthInLocalUnits = (remnantSeamWidthInFeet - AreaFinishBase.OverlapInInches) * 12.0 / Page.DrawingScaleInInches;

            materialWidthInLocalUnits = remnantSeamWidthInFeet * 12.0 / Page.DrawingScaleInInches;
        ...
        public double CutsAreaInSqrInches()
        {
        ...
        public void RemoveSeamsAndRollouts()
        {
            DeleteNonManualSeams();
        ...
        public void RemoveEmbeddedOvers()
        {
        ...
        public void SetAreaDesignStateFormat(/*AreaMode areaMode,*/ bool recursive = false)
        {
            Color color;
        ...
        public void SetSeamDesignStateFormat(SeamMode seamMode)
        {
#if true
            if (IsSubdivided())
            {
        ...
        public void SetCompletedLineWidth()
        {
            ExternalArea.SetLineGraphics(SystemState.DesignState, false, AreaShapeBuildStatus.Completed);
        }

        public void setupBorderAreaOverageOrUndrage()
        {
            double drawingScaleInInches = Page.DrawingScaleInInches;
            ...
            if (widthInInches <= rollWidthInInches * 0.5)
            {
                ...
                Undrage undrage = new Undrage(width, lngth, widthInInches, materialOverlapInInches);

                this.BorderAreaUndrage = new GraphicsUndrage(null, Window, Page, undrage, null);
        ...
        private DirectedLine getFixedWidthLine(double width)
        {
            ...
            {
                double length = canvasDirectedLine.Length * this.Page.DrawingScaleInInches;
        ...
        public void SetLineDesignStateFormat(/*LineMode lineMode*/)
        {
            if (!(this.SeamIndexTag is null))
            {
                this.SeamIndexTag.Delete();
...
//        public void SetupFromParentNewMethod(CanvasLayoutArea parentLayoutArea)
//        {
//#if TRACE0
...
//                    CurrentPage.AddToPageShapeDict(line);
//                }
...
//        }
        ...
        //public void AddSeamIndexTag(double x, double y)
        //{
        ...
        //public void AddSeamIndexTag(CanvasSeamTag seamIndexTag)
        //{
...
#region Constructors
        ...
        public CanvasLayoutArea(
            ...
            ): base(window, page, finishLibElements)
        {
                ...
                
        {
            ...
            if (this.LayoutAreaType == LayoutAreaType.FixedWidth)
            {
                this.BorderWidthInInches = SystemGlobals.FixedWidthScaleInInches();
                ...
                , finishLibElements)
        {
            ...
            this.ExternalArea = new CanvasDirectedPolygon(Window, Page, lineFinishManager, layoutArea.ExternalArea, originatingDesignState);
            ...
            {
                CanvasDirectedPolygon internalArea = new CanvasDirectedPolygon(Window, Page, lineFinishManager, ia, originatingDesignState);
        ...
        //    CurrentPage.RemoveLayoutArea(Guid);
        ...
        //    CurrentPage.AddLayoutArea(this);
        ...
        public void RemoveInternalAreas(List<GraphicShape> internalAreaShapes)
        {
            ...
            CurrentPage.RemoveLayoutArea(prevGuid);

            Page.RemoveFromPageShapeDict(prevGuid);

            Shape = Shape.Subtract(this, Window, Page, internalAreaShapes);

            Shape.SetShapeData("Layout Area Shape", "Compound Shape[" + this.AreaFinishManager.AreaName + "]", Shape.Guid);
        ...
        public void SetPatternFillColor(string visioFillColorFormula)
        {
        ...
        public void SetPatternFillTransparancy(string visioFillTransparencyFormula)
        {
            VisioInterop.SetPatternTransparency(this.Shape, visioFillTransparencyFormula);
        }
        ...
        public void AddBackToCanvas()
        {
            ...
            if (ExternalArea != null)
            {
                ...
                foreach (CanvasDirectedLine directedLine in ExternalArea)
                {
                    ...
                    CurrentPage.AddToDirectedLineDict(directedLine);
                }

                PolygonInternalArea.SetShapeData3(Guid);
            }
            ...
            this.SetLineGraphics(SystemState.DesignState, AreaShapeBuildStatus.Completed);
        ...
        /// </summary>
        public void RemoveFromCanvas(bool deleteAssociatedLines = false)
        {
#if TRACE0
            Tracer.TraceGen.TraceMethodCall(1, false, new object[] { });
#endif
            try
            {
                ...
                if (Utilities.IsNotNull(ExternalArea))
                {
                    ...
                    //foreach (CanvasDirectedLine directedLine in ExternalArea)
                    //{
                    //    CanvasPage.CurrentPage.RemoveDirectedLine(directedLine);
                    //}
                }
        ...
        //{
        //    CanvasPage currentPage = CanvasPage.CurrentPage;
        ...
        public void Delete(bool deleteAssociatedLines = false)
        {
            ...
            if (this.LayoutAreaType == LayoutAreaType.Remnant)
            {
                if (Utilities.IsNotNull(this.SeamIndexTag))
                {
                    ...
                    SystemGlobals.RemoveRemnantArea(seamAreaIndex);
                }
            }
            ...
            CurrentPage.RemoveLayoutArea(this);
        ...
        private CanvasLayoutArea ConvertToCanvasLayoutArea(LayoutArea layoutArea, UCLineFinishPaletteElement ucLine, DesignState designState)
        {
            ...
            CanvasDirectedPolygon externalPerimeter = new CanvasDirectedPolygon(
                this.Window, this.Page, graphicsDirectedPolygon, ucLine, designState);
            ...
            foreach (DirectedPolygon internalPerimeter in layoutArea.InternalAreas)
            {
                ...
                internalPerimeters.Add(new CanvasDirectedPolygon(
                    this.Window, this.Page, internalDirectedPolygon, ucLine, DesignState.Seam));
            }
        ...
        public UCLineFinishPaletteElement MaxLineType()
        {
            ...
            return PalettesGlobal.LineFinishPalette[maxGuid];
        }
        ...
        public GraphicShape Draw(GraphicsWindow window, GraphicsPage page)
        {
        ...
        public CanvasLayoutArea GetSibling()
        {
        ...
        public List<CanvasLayoutArea> GetSiblings()
        {
            Tracer.TraceGen.TraceMethodCall(1, false, new object[] { });
        ...
        public Dictionary<string, GraphicShape> GenerateGraphicShapeDict()
        {
            ...
            foreach (var x in this.SeamList)
            {
