...
namespace OversUndersForm
{
    ...
    public partial class UCCutsOversUndersFormElement : UserControl, IDisposable
    {
            ...
            , int extraInchesPerCut)
        {
            ...
            {
                double totlWdthInInches =  Math.Round(drawingScaleInInches * cut.CutRectangle.Height);

                if (Math.Abs(totlWdthInInches - rollWdthInInches) > 1e-1)
                {
                    MessageBox.Show("Invalid cut roll width found.");
                }
            }
            ...
            int hghtInTotlInchs = (int) Math.Round(drawingScaleInInches * cut.CutRectangle.Width) + extraInchesPerCut;
            int wdthInTotlInchs = (int) Math.Round(drawingScaleInInches * cut.MaterialWidth);
            ...
            {
                hghtInTotlInchs = (int)Math.Round(drawingScaleInInches * cut.OverrideCutRectangle.Height);
                wdthInTotlInchs = (int)Math.Round(drawingScaleInInches * cut.OverrideCutRectangle.Width);
            ...
            if (GlobalSettings.ValidateRolloutAndCutWidth)
            {
                ...
                int totlWdthInInches = (int) Math.Round(drawingScaleInInches * cut.CutRectangle.Height);

                if (Math.Abs(wdthFeet - totlWdthInInches / 12) > 1e-1)
                {
                    MessageBox.Show("Invalid cut width detected in textboxes.");
                }
            }
