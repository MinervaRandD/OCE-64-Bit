...
namespace FloorMaterialEstimator.OversUndersForm
{
    ...
    /// </summary>
    public partial class UCUndrsOversUndersFormElement : UserControl
    {
        public Undrage Undrage { get; set; }
        ...
        private OversUndersForm oversUndersForm = null;

        /// <summary>
        /// Overs / unders form element constructor
        /// </summary>
        public UCUndrsOversUndersFormElement()
        {
            InitializeComponent();
        }
        ...
        internal void Init(OversUndersForm oversUndersForm, GraphicsUndrage undrage, double drawingScaleInInches, int extraInchesPerCut)
        {
            this.oversUndersForm = oversUndersForm;

            this.Undrage = undrage;

            this.drawingScaleInInches = drawingScaleInInches;

            this.lblNmbr.Text = Utilities.IndexToUpperCaseString(undrage.UndrageIndex);

            int hghtInTotlInchs = (int)Math.Round(drawingScaleInInches * undrage.EffectiveDimensions.Item1) + extraInchesPerCut;
            int wdthInTotlInchs = (int)Math.Round(drawingScaleInInches * undrage.EffectiveDimensions.Item2);

            wdthInTotlInchs += (int) Math.Round(undrage.MaterialOverlap);

            int halfWidthInInches = (int)Math.Round(drawingScaleInInches * 0.5 * undrage.MaterialWidth);

            if (wdthInTotlInchs > halfWidthInInches)
            {
                wdthInTotlInchs = halfWidthInInches;
            }
            ...
            {
                hghtInTotlInchs = (int)Math.Round(drawingScaleInInches * undrage.OverrideEffectiveDimensions.Item1);
                wdthInTotlInchs = (int)Math.Round(drawingScaleInInches * undrage.OverrideEffectiveDimensions.Item2);
            ...
            this.Click += UCUndrsOversUndersFormElement_Click;
        }

        public bool Selected { get; set; } = false;

        private void UCUndrsOversUndersFormElement_Click(object sender, EventArgs e)
        {
            setSelectedStatus();
        }
        ...
        private void resetOverrideEffectiveDimensions(int lnthFeet, int lnthInches, int wdthFeet, int wdthInches)
        {
            ...
            {
                Undrage.OverrideEffectiveDimensions = null;
            ...
            else
            {
                ...
                Undrage.OverrideEffectiveDimensions =
        ...
        public new string Width => this.txbWdth.Text.Trim();

        public new string Length => this.txbLnth.Text.Trim();
    }
}
