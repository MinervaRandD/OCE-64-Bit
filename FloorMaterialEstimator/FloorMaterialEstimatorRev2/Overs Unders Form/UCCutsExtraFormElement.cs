...
namespace FloorMaterialEstimator.OversUndersForm
{
    ...
    public partial class UCCutsExtraFormElement : UserControl
    {
        ...
        private bool within_TxbLnth_TextChanged = false;

        private void TxbLnth_TextChanged(object sender, EventArgs e)
        {
            
            this.txbLnth.Text = this.txbLnth.Text.Replace("\r", "").Replace("\n", "").Replace("\t", "");

            if (string.IsNullOrWhiteSpace(this.txbLnth.Text))
            {
                SystemState.XtraLgthFeet = null; SystemState.XtraLgthInch = null;
            ...
            int? xtraLgthFeet = null;
            int? xtraLgthInch = null;

            if (!Utilities.CheckTextBoxValidMeasurement(this.txbLnth, out xtraLgthFeet, out xtraLgthInch, true))
            {
            ...
            SystemState.XtraLgthFeet = xtraLgthFeet;
            SystemState.XtraLgthInch = xtraLgthInch;
        ...
        public void Setup()
        {
            if (!SystemState.XtraLgthFeet.HasValue && !SystemState.XtraLgthInch.HasValue)
            {
                this.txbLnth.Text = string.Empty;

                return;
            }


            if (!SystemState.XtraLgthInch.HasValue)
            {
                SystemState.XtraLgthInch = 0;
            }

            if (!SystemState.XtraLgthFeet.HasValue)
            {
                SystemState.XtraLgthFeet = 0;
            }


            this.txbLnth.Text = SystemState.XtraLgthFeet.ToString() + "' " + SystemState.XtraLgthInch.ToString() + "\" ";
        ...
        {
            if (SystemState.XtraLgthFeet == null || SystemState.XtraLgthInch == null)
            {
                return 0;
            }

            return SystemState.XtraLgthFeet.Value * 12 + SystemState.XtraLgthInch.Value;
        }
