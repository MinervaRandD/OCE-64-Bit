...
namespace FloorMaterialEstimator.OversUndersForm
{
   ...
   // using OversUndersLibOldVersion;
    ...
    public partial class OversUndersForm : Form
    {
        ...
        List<GraphicsUndrage> undrageList = new List<GraphicsUndrage>();
        ...
        public void UpdatePanelDisplay(AreaFinishManager selectedElement, bool doUpdateTotals)
        {
            ...
            //areaFinishManager.GraphicsCutList = this.graphicsCutList;
        ...
        private void generateMaterialsLists(List<CanvasLayoutArea> layoutAreaList)
        {
            foreach (GraphicsLayoutArea layoutArea in layoutAreaList)
            {
                ...
                {
                    foreach (GraphicsUndrage undrage in layoutArea.UndrageList)
                    {
            ...
            graphicsCutList.Sort((c1, c2) => (int) c1.CutIndex - (int)c2.CutIndex);
        ...
        private void buildOversPanel(List<Overage> overageList, double drawingScale)
        {
            ...
            loclOverageList.Sort((o1, o2) => (int) o1.OverageIndex - (int)o2.OverageIndex);
        ...
        private void buildUndrsPanel(List<GraphicsUndrage> undrageList, double drawingScaleInInches)
        {
            List<GraphicsUndrage> loclUndrageList = new List<GraphicsUndrage>(undrageList);

            loclUndrageList.Sort((u1, u2) => (int) u1.UndrageIndex - (int)  u2.UndrageIndex);
            ...
            foreach (GraphicsUndrage undrage in loclUndrageList)
            {
                if (undrage.Deleted)
                {
                    continue;
        ...
        private void buildVirtualOversPanel(List<VirtualOverage> virtualOverageList, double drawingScaleInInches)
        {
            ...
            foreach (VirtualOverage overage in virtualOverageList)
            {
                ...
                oversOversUndersFormElement.Init(this, (uint) count, overage, drawingScaleInInches);
        ...
        private void buildVirtualUndrsPanel(List<VirtualUndrage> virtualUndrageList, double drawingScaleInInches)
        {
            ...
            foreach (VirtualUndrage undrage in virtualUndrageList)
            {
                ...
                undrsOversUndersFormElement.Init(this, (uint)count, undrage, drawingScaleInInches);
