    ...
    public partial class CombosBaseForm : Form, IMessageFilter
    {
        ...
        private void initDisplay()
        {
            ...
            foreach (CanvasLayoutArea layoutArea in ucAreaFinish.CanvasLayoutAreas)
            {
                ...
                foreach (CanvasCut cut in layoutArea.CanvasCutList)
                {
                    if (cut.CutPolygon.AreaInSqrInches(currentPage.DrawingScaleInInches) / cut.Rollout.AreaInSqrInches(currentPage.DrawingScaleInInches) >= .5)
                    {
                        cutList.Add(cut);
