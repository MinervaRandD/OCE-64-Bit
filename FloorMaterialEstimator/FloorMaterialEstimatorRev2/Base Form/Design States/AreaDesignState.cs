...
//        }
        ...
        public void lblFixedWidthJump_Click(object sender, EventArgs e)
        {
            CanvasManager.ProcessFixedWidthCompleteShape();
        }
        ...
        public void BtnEditAreaCopyAndPasteShapes_Click(object sender, EventArgs e)
        {
            if (Utilities.IsNotNull(CanvasManager.CopySelectedLayoutArea))
            {
                ...
                {
                    CurrentPage.RemoveFromPageShapeDict(copyMarker.Shape);

                    var graphicsLayerBase = CanvasManager.CopySelectedLayoutArea.AreaFinishManager.AreaDesignStateLayer;

                    bool isLocked = graphicsLayerBase.IsLocked();
                    ...
                    graphicsLayerBase.RemoveShapeFromLayer(copyMarker, 1);
        ...
        public void SetAreaCopyAndPasteShapesStatus(bool status)
        {
            if (status == false)
            {
                if (Utilities.IsNotNull(CanvasManager.CopySelectedLayoutArea))
