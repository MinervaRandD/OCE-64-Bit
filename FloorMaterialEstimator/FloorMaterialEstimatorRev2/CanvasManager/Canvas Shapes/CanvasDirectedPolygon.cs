    ...
    public class CanvasDirectedPolygon : GraphicsDirectedPolygon, IAreaShape
    {
        ...
        public CanvasDirectedPolygon(CanvasManager canvasManager, DirectedPolygon polygon, DesignState designState): base(canvasManager.Window, canvasManager.CurrentPage)
        {
            ...
            foreach (DirectedLine directedLine in polygon)
            {
                ...
                CanvasDirectedLine canvasDirectedLine = directedLine as CanvasDirectedLine;

                LineFinishManager lineFinishManager = null;

                if (canvasDirectedLine != null)
                {
                    lineFinishManager = canvasDirectedLine.LineFinishManager;
                }

                else
                {
                    lineFinishManager = FinishManagerGlobals.SelectedLineFinishManager;

                }

                CanvasDirectedLine canvasDirectedLine1 = new CanvasDirectedLine(canvasManager, lineFinishManager, graphicsDirectedLine, designState);

                PerimeterAdd(canvasDirectedLine1);
            }
        ...
        internal GraphicShape CreateInternalAreaShape()
        {
            double[] coordinateArray = GetCoordinateArray();

            this.Guid = GuidMaintenance.CreateGuid(this);

            Shape = canvasManager.CurrentPage.DrawPolygon(this, coordinateArray, this.Guid);
        ...
        internal GraphicShape CreateInternalAreaShape(string guid)
        {
            double[] coordinateArray = GetCoordinateArray();

            Shape = canvasManager.CurrentPage.DrawPolygon(this, coordinateArray, guid);
        ...
        internal GraphicShape CreateInternalAreaShape(GraphicsPage page)
        {
            double[] coordinateArray = GetCoordinateArray();
        ...
        internal GraphicShape Draw(GraphicsPage page, Color color, bool drawPolygonOnly = false)
        {
            ...
            Shape = page.DrawPolyline((GraphicsDirectedPolygon) this, coordinates, 1);


            if (this.AreaFinishBase.Pattern == 0)
            {
                VisioInterop.SetBaseFillColor(Shape, this.AreaFinishBase.Color);
                VisioInterop.SetFillOpacity(Shape, this.AreaFinishBase.Opacity);
            }

            else
            {
                VisioInterop.SetFillPattern(Shape, this.AreaFinishBase.Pattern.ToString(), this.AreaFinishBase.Color);
                VisioInterop.SetPatternOpacity(Shape, this.AreaFinishBase.Opacity);
            }
