...
namespace FloorMaterialEstimator.CanvasManager
{
    ...
    public class AreaFinishManager : IDisposable
    {
        ...
        public GraphicsLayerBase AreaPerimeterLayer => AreaFinishLayers.AreaPerimeterLayer;
        ...
        //public GraphicsLayerBase SeamDesignStateLockLayer => AreaFinishLayers.SeamDesignStateLockLayer;
        ...
        public void AddLabel(GraphicsLabel graphicsLabel)
        {
            ...
            AreaFinishLayers.AreaFinishLabelLayer.AddShape(graphicsLabel.Shape, 1);
        }
        ...
        public List<VirtualOverage> VirtualOverageList
        {
            get;
            set;
        } = new List<VirtualOverage>();
        ...
        public int Pattern => AreaFinishBase.Pattern;
        ...
        internal void UpdateFinishStats(bool basic = false)
        {
            ...
            int count = 0;
            double? grossAreaInSqrInches = null;

            foreach (CanvasLayoutArea canvasLayoutArea in CanvasLayoutAreas)

            {   if (canvasLayoutArea.OffspringAreas.Count > 0 || canvasLayoutArea.IsZeroAreaLayoutArea)
                {
                    continue;
                }

                double? grossArea = canvasLayoutArea.GrossAreaInSqrInches();

                if (grossArea.HasValue)
                {
                    if (!grossAreaInSqrInches.HasValue)
                    {
                        grossAreaInSqrInches = 0;
                    }

                    count++;

                    grossAreaInSqrInches+= grossArea.Value;
                }
            }

            count = CanvasLayoutAreas.Count(a =>
                    a.LayoutAreaType != LayoutAreaType.OversGenerator
                    && a.LayoutAreaType != LayoutAreaType.ColorOnly
                    && !a.IsSubdivided()
                    && a.IsVisible);

            double? wasteRatio = null;

            if (this.MaterialsType == MaterialsType.Rolls)
            {
                if (IsSeamed())
                {
                    wasteRatio = calculateRollWasteFactor(netAreaInSqrInches, perimeterInInches);
                }
            }
            ...
            if (basic)
            {
                AreaFinishBase.UpdateFinishStatsBasic(count, netAreaInSqrInches, grossAreaInSqrInches, perimeterInInches, wasteRatio);
            }
        ...
        private double? calculateRollWasteFactor(double netAreaInSqrInches, double perimeterInInches)
        {
            OversMaterialAreaList.Clear();
            UndrsMaterialAreaList.Clear();

            RolloutStatsCalculator rolloutStatsCalculator = new RolloutStatsCalculator(this.page, AreaFinishBase);
            double? wasteRatio = rolloutStatsCalculator.CalculateRolloutStats(CanvasLayoutAreas);
            ...
            , AreaFinishBase areaFinishBase)
        {
            ...
            this.AreaFinishBase.NetAreaChanged += AreaFinishBase_NetAreaChanged;

            this.AreaFinishBase.GrossAreaChanged += AreaFinishBase_GrossAreaChanged;
        ...
        private void AreaFinishBase_NetAreaChanged(AreaFinishBase finishBase, double netArea)
        {
            ...
            // DebugUtils.ValidateNetVsGross(this.IsSeamed(), NetAreaInSqrInches, GrossAreaInSqrInches);
        }

        private void AreaFinishBase_GrossAreaChanged(AreaFinishBase finishBase, double? grossArea)
        {
            if (SystemState.LoadingExistingProject)
            {
                return;
        ...
        public void UpdateBaseColor(Color updtColor)
        {
            ...
            else
            {
                foreach (CanvasLayoutArea canvasLayoutArea in CanvasLayoutAreas)
                {
                    canvasLayoutArea.SetPatternColor(visioFillColorFormula);
                }
            }
        }
        ...
        {
            double transparancy = 100.0 - Math.Max(Math.Min(100.0 * opacity, 100.0), 0.0);

            string visioFillTransparencyFormula = transparancy.ToString("0.00000000") + "%";
            ...
            else
            {
                foreach (CanvasLayoutArea canvasLayoutArea in CanvasLayoutAreas)
                {
                    if (canvasLayoutArea.PatternFill is null)
                    {
                        continue;
                    }

                    canvasLayoutArea.PatternFill.UpdateOpacity(opacity);
                }
            }
        }
        ...
        public void UpdatePattern(int pattern)
        {
            ...
            foreach (CanvasLayoutArea canvasLayoutArea in CanvasLayoutAreas)
            {
                canvasLayoutArea.SetFillPattern(Color, pattern);
            }
        ...
        public void SetDesignStateFormat(DesignState designState, SeamMode seamMode, bool filtered, bool showAreas)
        {
            ...
            if (designState == DesignState.Area)
            {
               ...
               // AreaFinishLayers.SeamDesignStateLockLayer.SetLayerVisibility(false);
            ...
            {
                AreaDesignStateLayer.SetLayerVisibility(false);
                AreaPerimeterLayer.SetLayerVisibility(false);
                SeamDesignStateLayer.SetLayerVisibility(false);
            ...
            {
                AreaDesignStateLayer.SetLayerVisibility(false);
                AreaPerimeterLayer.SetLayerVisibility(true);
                SeamDesignStateLayer.SetLayerVisibility(true);


               // AreaFinishLayers.SeamDesignStateLockLayer.SetLayerVisibility(SystemState.RbnShowAllLocks.Checked && this.Selected);
        ...
        internal void AddNormalLayoutAreaSeamStateOnly(CanvasLayoutArea canvasLayoutArea, bool updateColor = true, bool updateStats = true)
        {
            addNormalLayoutArea(canvasLayoutArea, updateColor, updateStats);

            canvasLayoutArea.AddToLayer(this.SeamDesignStateLayer.GetBaseLayer());
        }

        internal void AddNormalLayoutAreaAreaStateOnly(CanvasLayoutArea canvasLayoutArea, bool updateColor = true, bool updateStats = true)
        {
            addNormalLayoutArea(canvasLayoutArea, updateColor, updateStats);

            this.AreaDesignStateLayer.AddShape(canvasLayoutArea.Shape, 1);
            //canvasLayoutArea.AddToLayer(AreaDesignStateLayer);
        }

        internal void AddNormalLayoutArea(CanvasLayoutArea canvasLayoutArea, bool recursive = true, bool updateColor = true, bool updateStats = true)
        {
            addNormalLayoutArea(canvasLayoutArea, updateColor, updateStats);

            //canvasLayoutArea.AddToLayer(AreaFinishManager.SeamDesignStateLayer.GetBaseLayer());

            this.SeamDesignStateLayer.AddShape(canvasLayoutArea.Shape, 1);
            this.AreaDesignStateLayer.AddShape(canvasLayoutArea.Shape, 1);

            foreach (var perimeterLine in canvasLayoutArea.Perimeter)
            {
                this.AreaPerimeterLayer.AddShape(perimeterLine.Shape, 1);
            }

            //canvasLayoutArea.AddToLayer(AreaDesignStateLayer);

            if (canvasLayoutArea.FillPattern != null)
            {
                canvasLayoutArea.FillPattern.AddToLayer(this.AreaDesignStateLayer);
                canvasLayoutArea.FillPattern.AddToLayer(this.SeamDesignStateLayer);
            }
            ...
            {
                AddNormalLayoutArea(offspringArea, recursive, updateColor, updateStats);
            }
        }

        private void addNormalLayoutArea(CanvasLayoutArea canvasLayoutArea, bool updateColor = true, bool updateStats = true)
        {
           ...
           // canvasLayoutArea.UCAreaFinish = this.UCAreaPaletteElement;
            ...
            if (updateColor)
            {
                ...
                {
                    canvasLayoutArea.SetFillPattern(this.AreaFinishBase.Color, this.AreaFinishBase.Pattern);
                    canvasLayoutArea.SetPatternOpacity(this.AreaFinishBase.Opacity);
                }
            }
            ...
            // The following locks the external lines so you cannot selected it. Careful programming probably will reveal
            // that it duplicates locks in some cases, but apparently it is necessary when you subdivide regions

            foreach (CanvasDirectedLine canvasDirectedLine in canvasLayoutArea.Perimeter)
            {
        ...
        internal void AddRemnantLayoutArea(CanvasLayoutArea canvasLayoutArea)
        {
            ...
            this.AreaFinishLayers.RemnantSeamDesignStateLayer.AddShape(canvasLayoutArea.PolygonInternalArea, 1);
        ...
        internal void SetupAllSeamLayers()
        {
            ...
            if (SystemState.DesignState == DesignState.Seam)
            {
                AreaFinishLayers.SetupAllSeamLayers(
                    ...
                    , SystemState.SeamModeAutoSeamsShowUnHideable
                    , SystemState.SeamModeManualSeamsShowAll
                    ...
                    , SystemState.ShowSeamAreaNmbrs);

                return;
            }
        }

        internal void SetupCutNumbering()
        {
            List<CanvasLayoutArea> orderedSeamedCanvasLayoutAreas = generateSeamedCanvasLayoutAreaOrderedByLocation();

            if (orderedSeamedCanvasLayoutAreas is null)
            {
                return;
            }
            ...
            foreach (CanvasLayoutArea canvasLayoutArea in orderedSeamedCanvasLayoutAreas)
            {         
                canvasLayoutArea.UpdateCutIndicesAndVisibility(ref seamIndx);
            }

        }

        private List<CanvasLayoutArea> generateSeamedCanvasLayoutAreaOrderedByLocation()
        {
            List<Tuple<Coordinate, CanvasLayoutArea>> seamedLayoutAreaList = new List<Tuple<Coordinate, CanvasLayoutArea>>();
            ...
            foreach (CanvasLayoutArea canvasLayoutArea in CanvasLayoutAreas)
            {
                ...
                //Coordinate upperLeftMostPoint = canvasLayoutArea.UpperLeftmostPoint();

                seamedLayoutAreaList.Add(new Tuple<Coordinate, CanvasLayoutArea>(canvasLayoutArea.UpperLeftmostPoint(), canvasLayoutArea));
            }

            if (seamedLayoutAreaList.Count <= 0)
            {
                return null;
            }

            seamedLayoutAreaList.Sort((c1, c2) => sortValue(c1.Item1, c2.Item1));

            return seamedLayoutAreaList.Select(c => c.Item2).ToList();
        }

        private static int sortValue(Coordinate c1, Coordinate c2)
        {
        ...
        public static implicit operator CanvasManagerLib.FinishManager.AreaFinishManager(AreaFinishManager areaFinishManager)
        {
            return new CanvasManagerLib.FinishManager.AreaFinishManager(
                areaFinishManager.window
                , areaFinishManager.page
                , areaFinishManager.AreaFinishBaseList
                , areaFinishManager.AreaFinishBase);
        }

        //internal void ShowLocks()
        //{
        //    SeamDesignStateLockLayer.SetLayerVisibility(true);
        //}

        //internal void HideLocks()
        //{
        //    SeamDesignStateLockLayer.SetLayerVisibility(false);
        //}

        //internal void DeleteLocks()
        //{
        //    foreach (CanvasLayoutArea canvasLayoutArea in this.CanvasLayoutAreas)
        //    {
        //        canvasLayoutArea.DeleteLock();
        //    }
        //}

        //internal void SetSeamDesignStateLockLayerVisiblity(bool visible)
        //{
        //    if (visible)
        //    {
        //        ShowLocks();
        ...
        //    foreach (CanvasLayoutArea canvasLayoutArea in this.CanvasLayoutAreas)
        //    {
        //        canvasLayoutArea.initializeSeamStateForExistingProject();
        //    }
        //}
    }
}
