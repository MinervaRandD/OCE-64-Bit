...
namespace CanvasManager.Reports.SummaryReport
{
    ...
    public partial class NonRollRow : UserControl, IDisposable, IReportRow
    {
        ...
        public void UpdateStatsDisplay(bool scaleHasBeenSet)
        {
            ...
            {
                this.lblTotal.Text = grsTotalInSqrFeet.ToString("#,##0") + " s.f.";
                this.lblNet.Text = netTotalInSqrFeet.ToString("#,##0") + " s.f.";
            }

            else
            {
                this.lblTotal.Text = (grsTotalInSqrFeet / 9.0).ToString("#,##0") + " s.y.";
