...
namespace FloorMaterialEstimator.CanvasManager
{
    ...
    public partial class CanvasManager
    {
            ...
            try
            {
                ...
                else
                {
                    if (BuildingPolyline.CoordList.Count > 0)
                    {
                        ...
                        if (MathUtils.H2Distance(lastCoord.X, lastCoord.Y, x, y) > 1.0e-1)
                        {
                            if (KeyboardUtils.RightBracketKeyPressed)
                            {
                                completeClockwiseCornerClick(x, y);
                                return;
                            }

                            if (KeyboardUtils.LeftBracketKeyPressed)
                            {
                                completeCounterClockwiseCornerClick(x, y);
                                return;
                            }
                        }
                    }
        ...
        private bool completeClockwiseCornerClick(double x3, double y3)
        {
            if (buildingPolyline.CoordList == null)
            {
            ...
            double x1 = lastCoord.X;
            double y1 = lastCoord.Y;

            double dx = x3 - x1;
            double dy = y3 - y1;

            if (Math.Abs(dx) <= 1.0e-1 && Math.Abs(dy) <= 1.0e-1)
            {
                return false;
            }
            ...
            // Need to avoid intersection at end points...
            ...
            if (y1p == y2p)
            {
                if (x1p > x2p)
                {
                    Utilities.Swap(ref x1p, ref x2p);
                }
            ...
            else if (x1p == x2p)
            {
                if (y1p > y2p)
                {
                    Utilities.Swap(ref y1p, ref y2p);
                }
            ...
            if (!buildingPolyline.ValidateNoIntersection(x1p, y1p, x2p, y2p) || !buildingPolyline.ValidateNoIntersection(x2, y2, x3, y3))
            {
                MessageBox.Show("Cannot complete this operation as it creates an invalid border.");
                return false;
            }

            buildingPolyline.AddPoint(x2, y2);

            buildingPolyline.AddPoint(x3, y3);

            if (MathUtils.H2Distance(frstCoord.X, frstCoord.Y, x3, y3) <= 1.0e-1)
            {
        ...
        private bool completeCounterClockwiseCornerClick(double x3, double y3)
        {
            if (buildingPolyline.CoordList == null)
            {
                return false; // defensive;
        ...
        //private bool completeBoxClick(double x3, double y3)
        //{

        //    double x1 = buildingPolyline.CoordList[0].X;
        //    double y1 = buildingPolyline.CoordList[0].Y;

        //    double dx = x3 - x1;
        //    double dy = y3 - y1;

        //    if (Math.Abs(dx) <= 1.0e-2 || Math.Abs(dy) <= 1.0e-2)
        //    {
        //        MessageBox.Show("Cannot complete box because points are either too close or in alignment.");
        //        return false;
        //    }
            
        //    bool rslt = completeClockwiseCornerClick(x3, y3);
        ...
        //    rslt = completeClockwiseCornerClick(x1, y1);
        ...
        public void ProcessFixedWidthCompleteShape()
        {
            ...
            foreach (Tuple<AreaFinishBase, List<Coordinate>> directedPolylineElement in directedPolylineElementList)
            {
                ...
                AreaFinishManager areaFinishManager = FinishManagerGlobals.AreaFinishManagerList[areaFinishBaseIndex];
        ...
        public void ProcessPolylineCompleteShape(bool fromButtonClick = false)
        {
            ...
            {
                ManagedMessageBox.Show("At least 3 points must be selected to complete the current drawing.");
        ...
        public void RemoveBuildingPolyLineBuildingLine()
        {
            if (SystemState.DrawingMode != DrawingMode.Polyline || !SystemState.DrawingShape || buildingPolyline is null)
            {
                ...
                if (layoutArea == this.CopySelectedLayoutArea)
                {
                    this.CopySelectedLayoutArea = null;
                }
                ...
                if (layoutArea.InternalAreas.Count > 0)
                {
                    ...
                    GraphicShape internalShape = CurrentPage.DrawPolygon(null, internalAreaPolygon.CoordinateArray());

                    layoutArea.AddBackInternalAreas(new List<GraphicShape> { internalShape });
